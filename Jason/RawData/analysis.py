...
inputname = str(input("Enter season [i.e. season1]: "))
filename = f"Raw_Data/{inputname}.json"
...
# Create a new column 'Season Ep' [S01E01]
SeasonEp = []
for title in df["Ep"]:
    epname = title[:6]
    SeasonEp.append(epname)
# print(len(SeasonEp))
df["SeasonEp"] = SeasonEp

mean_df = df[["SeasonEp","Compound"]].groupby(["SeasonEp"]).agg('mean') #.sort_values(ascending=False)

# Output a csv file
mean_df.to_csv(f"{inputname}.csv", sep=",", encoding='utf-8', index=True)
